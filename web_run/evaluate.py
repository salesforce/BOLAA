...
def get_eval_values(llm_name, agent_type, path="./execution_data/"):
    ...
    # complexity = eval_complexity(sessions, complexity_dict)
    success = eval_success(sessions)
    fail = eval_fail(sessions)
    # recall_to_comp = y_wrt_x_rel(complexity, recalls)
    # reward_to_comp = y_wrt_x_rel(complexity, rewards)
    # prec_to_comp = y_wrt_x_rel(complexity, precisions)
